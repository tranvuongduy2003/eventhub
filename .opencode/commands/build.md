...
> **EPHEMERAL PLAN:** `.opencode/plans/` only. **Never commit.** **Delete** when all tasks complete.
...
| Input | Action |
|-------|--------|
| `.opencode/plans/<file>.md` | Use that plan |
| `docs/specs/<file>.md` | Use paired `.opencode/plans/<same-filename>.md` |
| Newest in `.opencode/plans/` | If no path |
...
@codebase-explorer — Find handlers and tests for …
```
...
| Command | Action |
|---------|--------|
| `/build .opencode/plans/20260614143000-place-order.md` | Full plan |
...
- Commit or stage `.opencode/plans/**`
