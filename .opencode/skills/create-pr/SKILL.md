---
name: create-pr
description: End-to-end pull request workflow — branch, local CI, Conventional Commits, push, open PR, optionally link GitHub issues (Closes #N), attach Project/labels/assignees on PR and issues, and set Project Status. Use when the user asks to create PR, open pull request, ship changes, run create-pr, or mentions attaching an issue to a PR.
---
...
| `labels a,b` | — | Extra PR labels, comma-separated |
| `assignee x` | — | Extra PR assignee (e.g. `assignee @me`) |
...
**Read and follow** [`.opencode/skills/git-commit-writer/SKILL.md`](../git-commit-writer/SKILL.md):
...
2. **Read and follow** [`.opencode/skills/pr-description-writer/SKILL.md`](../pr-description-writer/SKILL.md) to draft the body from `git diff main...HEAD` and `git log --oneline main..HEAD`.
...
.opencode/skills/create-pr/scripts/sync-pr-github-metadata.ps1 @params
```
...
| Implement before PR | `build` command in [`.opencode/commands/build.md`](../../commands/build.md) |
