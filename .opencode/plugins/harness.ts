...
type HookResult = { exitCode: number; json: Record<string, unknown> | null; stdout: string }
...
async function invokeHook(
  ...
  const result = await $`pwsh -NoProfile -File ${scriptPath}`
    .env({
      ...process.env,
...
function cursorToolInput(tool: string, args: Record<string, unknown>): Record<string, unknown> {
  if (tool === "bash") {
  ...
  return {
    "tool.execute.before": async (input, output) => {
      ...
      const hookInput = {
        ...
        tool_input: cursorToolInput(tool, (output.args ?? {}) as Record<string, unknown>),
      ...
      if (tool === "bash") {
        ...
        const shellResult = await invokeHook($, beforeShellGuard, shellInput, root)
        if (shellResult.exitCode === 2 && shellResult.json?.permission === "deny") {
          const message =
    ...
    "experimental.session.compacting": async (input) => {
      const sessionInput = {
        conversation_id: (input as { sessionID?: string }).sessionID ?? "unknown",
        trigger: "compaction",
