---
description: "Execute .claude/plans/ tasks with evaluator-optimizer loop. Parent alone writes production code; parallel readonly scouts; build-test-writer for bugs. Verify with real tests/types until green. Delete plan when done."
argument-hint: "[plan-or-spec-file] [task N]"
...
> **EPHEMERAL PLAN:** `.claude/plans/` only. **Never commit.** **Delete** when all tasks complete.
...
Constitution · `ddd.md` · `technical.md` · **`architecture.md`** · spec from plan `related_spec` · `.claude/notes/progress.md`.

## Step 1: Parse input

| Input | Action |
|-------|--------|
| `.claude/plans/<file>.md` | Use that plan |
| `docs/specs/<file>.md` | Use paired `.claude/plans/<same-filename>.md` |
| Newest in `.claude/plans/` | If no path |
...
Merge summaries into `.claude/notes/progress.md` **Decisions**. **Do not** let subagents edit code.
...
| Command | Action |
|---------|--------|
| `/build .claude/plans/20260614143000-place-order.md` | Full plan |
