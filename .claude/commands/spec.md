---
description: "Write one implementation-ready product spec to docs/specs/ from feature id (F-*) or user description. No file paths or class names. Output feeds /plan."
...
Follow `.claude/rules/spec-artifacts.md`.
...
Use `AskUserQuestion` only if you would otherwise make >3 major assumptions. Do not ask implementation questions.
...
**Body must include** (or copy from `.github/.issue-bodies/spec.template.md`):
...
- Write implementation plans in the spec (`/plan` writes `.claude/plans/` — gitignored)
