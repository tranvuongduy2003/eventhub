---
description: "Five-layer agent walkthrough — end-to-end example showing how Context, Harness, Reasoning, Workflow, and Topology layers work together for a typical field change (add Phone to User). Read when implementing non-trivial changes to understand the full agent orchestration pattern. --- # Five-layer walkthrough — add `Phone` to `User`"
Ad-hoc ask: *"Add field `phone` to model `User`."*  
...
```text
Agent(graph-impact-analyst, prompt="blast radius: add Phone to User aggregate F-* …")
Agent(codebase-explorer, prompt="User aggregate, persistence mapper, API users endpoints, tests")
```
...
Parent records in `.claude/notes/progress.md` **Goal** / **Decisions**:
...
2. **Red test first** (behavior change): `Agent(build-test-writer, …)` — e.g. API/register response includes `phone` or domain invariant test; confirm **red once**.
3. **Single writer:** parent applies minimal diff — `User`, mapper, migration, OpenAPI, endpoint mapping.
4. Agent **cannot** self-declare done — only green checks count (`reasoning-loop` section in CLAUDE.md).

---

## Layer 2 — Harness (deterministic)

On each **Write**, `PostToolUse` → `post-edit-verify.ps1`:
...
3. **Verify gate** set → `pre-tool-guard` / `before-shell-guard` block `Bash`/`Agent` until fix saved and reverified.
...
**Stop gate** (`stop-gate.ps1`): on stop, full `git diff` + `tsc` (if web) + deduped affected steps must pass → else `followup_message` forces continue.
...
|-------|--------|-----------|
| Scout | `graph-impact-analyst`, `codebase-explorer`, `test-impact-analyzer` | Yes (readonly) |
| Red test | `build-test-writer` | No — alone |
| Production code | **Parent** | No — never parallel writers |
...
**PreCompact** hook backs up `progress.md` + transcript to `.claude/notes/backups/`.
