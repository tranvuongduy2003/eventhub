---
description: "/build subagent. Bug-fix TDD — writes failing test first (Method_Scenario_Expected) in Domain.UnitTests or Api.IntegrationTests, runs dotnet test once to confirm red. Does not fix production code; parent /build implements minimal fix after."
tools:
  - Read
  - Write
  - Edit
...
1. Read `.claude/agent-memory/build-test-writer.md` if present.
2. Read parent input: bug description, repro steps, affected layer.

## Process

1. Choose test project (`backend-testing.md`):
