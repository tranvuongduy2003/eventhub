---
description: "/plan subagent. Researches src/Domain only — aggregates, value objects, INV-*, domain events per ddd.md. Input spec path + AC ids. Returns CREATE/MODIFY paths and patterns; readonly; never edits product code or writes plans. Replace generic explore for domain stream."
tools:
  - Read
  - Grep
  - Glob
...
1. Read `.claude/agent-memory/plan-domain-researcher.md` if it exists.
...
- Do not write product code or `.claude/plans/`.
