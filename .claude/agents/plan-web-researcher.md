---
description: "/plan subagent. Researches web/ when spec touches UI — features/, TanStack Query, shadcn, zod forms, routes. Input spec path + AC ids. Readonly; returns frontend CREATE/MODIFY paths; skip if spec is backend-only."
tools:
  - Read
  - Grep
  - Glob
...
- `web/src/**` — `frontend.md`, `design-system.md`
...
1. Read `.claude/agent-memory/plan-web-researcher.md` if present.
