...
When Claude Code compacts context (auto or manual), `.claude/hooks/pre-compact-backup.ps1` runs and saves under `backups/<timestamp>/`:

- `precompact-meta.json` — token usage, trigger
...
- Worker-layer memory: `.claude/agent-memory/<agent-name>.md` (one file per project subagent)
- Agent harness: see `CLAUDE.md` (Context memory and Agent harness sections)
