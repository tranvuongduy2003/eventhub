...
```json
{
  ...
  "run": {
    "type": "hook | command",
    "script": ".opencode/hooks/pre-tool-guard.ps1",
...
Manual **agent** cases (`mode: manual`) define prompts + post-conditions for OpenCode sessions; skipped unless `-IncludeAgent`.
