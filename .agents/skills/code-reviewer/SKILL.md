...
**webhook-handler.ts:42** â€” The request body is passed directly to
...
**webhook-handler.ts:67** â€” The retry logic uses a fixed 1-second
delay. Consider exponential backoff to avoid hammering the downstream
service during outages.

**webhook-handler.ts:89** â€” The `processEvent` function handles both
