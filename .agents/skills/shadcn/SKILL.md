...
- **No Unicode text characters as visual markers.** `•`, `→`, `✓`, `×`, `›`, etc. used as bullets/arrows/separators/indicators become icon components (`<Dot />`, `<ArrowRight />`, `<Check />`, `<X />`, `<ChevronRight />`). Real prose punctuation (`—`, `…` as sentence ender) is fine.
