...
**Domain model alignment (from ddd.md):**
...
- The `ticket_types` table already has a `max_per_order` column (nullable integer) from the initial schema (ddd.md ENT-TicketType). No schema change is required.
...
**Consistency:**
- The per-order limit is checked at order placement time, within the same transaction that reserves inventory (the pragmatic two-aggregate write described in ddd.md §8). There is no eventual-consistency concern — the limit is a point-in-time validation.
