...
- **Transparent pricing (F-3.3):** The price displayed on the public page is the final price. No fees, surcharges, or taxes are added later in the purchase flow. This is a core product promise (`prd.md` QG-2, DEC-1).
...
- **Guest access:** The public page is a read-only surface that requires no authentication. This aligns with the attendee lifecycle (`prd.md` §5.2): find the event → see the price → buy.
...
- The currency is the single configured currency for the platform (no multi-currency — `prd.md` §6.2).
- The public endpoint does not require rate limiting for MVP given the small event scale (`prd.md` ASM-2).
