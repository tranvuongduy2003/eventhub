...
1. **This constitution** wins over all other documents and OpenCode rules.
2. **Product and technical docs** (`prd.md`, `features.md`, `ddd.md`, `technical.md`) win over scoped OpenCode rules.
...
.opencode/   Rules, skills, commands; ephemeral plans in `.opencode/plans/` (gitignored)
```

Do not collapse layers into monolithic projects or move orchestration outside AppHost.

**Durable workflow artifact:** specs in `docs/specs/` (`YYYYMMDDHHmmss-<name>.md`). **Ephemeral plans** in `.opencode/plans/` — same basename as spec; never committed; deleted after `/build`.
...
2. Update affected docs (`prd.md`, `features.md`, `ddd.md`, `technical.md`, OpenCode rules) to match.
