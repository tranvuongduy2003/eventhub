...
1. The User aggregate can hold both organizer and attendee roles (via `VO-UserRole` as defined in domain-model-specification.md). The current aggregate is a template; the role field is added as part of this feature — no migration concern.
2. Guest orders store the buyer's email in a queryable field (`Contact.Email` on the Order aggregate). This is already the case per domain-model-specification.md BC-3.
