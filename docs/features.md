...
### 0.1 Purpose
This document defines **the capabilities of EventHub** — the epics, the features inside them, who each serves, and how each is judged done. It is the *what*. The *why/who/scope* lives in [`prd.md`](prd.md); the *how it is built* lives in [`technical.md`](technical.md).
...
- **Phases** describe when a feature is built, not a calendar date (the project is milestone-based — see `prd.md` DEC-5):
  - **MVP** — the smallest end-to-end product: run one small event from creation to entry (per `prd.md` DEC-3).
  - **Next** — richer organizing, discovery, audience tools, and fair transfer.
  - **Later** — messaging, returns, and enhancements.
- **Personas** (defined in `prd.md` §4): `PER-O1` individual organizer · `PER-O2` small group / club / small-business organizer · `PER-A1` general attendee · `PER-A2` group buyer.
- **Glossary** of shared terms is in `prd.md` §12 and is not repeated here.
- **Acceptance criteria** state the observable behavior that marks a feature complete. They are intentionally implementation-free; mechanisms (aggregates, ports, infrastructure) are in `technical.md`.
...
- *Note:* avatar is an image asset (stored as described in `technical.md` §5).
...
- **Acceptance criteria**
  ...
  - A free event is free to create and to attend (`prd.md` DEC-1).
...
- **Acceptance criteria**
  ...
  - No platform fee is added at any step (`prd.md` DEC-1).
...
- *Note:* the consistency mechanism is in `technical.md` §6.
...
- **Acceptance criteria**
  - On a typical phone screen, the page is readable and the buy flow is usable without horizontal scrolling or zooming (`prd.md` QG-4).
...
- **Acceptance criteria**
  ...
  - EventHub never collects or stores card details (`prd.md` QG-6).
...
- **Acceptance criteria**
  - The amount charged equals the displayed total with no EventHub fee added; funds settle to the organizer through the provider, not through EventHub (`prd.md` DEC-1).
...
- **Acceptance criteria**
  ...
  - Delivery happens reliably even if it is processed shortly after confirmation (handled asynchronously — see `technical.md` §5).
...
**Builds on previous:** acts on the tickets from EP-7 and, for returns, the inventory from EP-3 and refunds from EP-6. Realizes `prd.md` DEC-2.
...
- **Acceptance criteria**
  ...
  - No money is collected by EventHub for a transfer, and there is no way to set a price — transfers are face value only (`prd.md` DEC-2).
...
*This document is a living draft for a personal project. It is the feature reference for EventHub; product intent is in `prd.md` and technical design is in `technical.md`. Phases and dependencies describe the intended build order and may be revised as the project evolves.*
