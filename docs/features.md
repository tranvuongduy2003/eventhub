...
# EventHub — Feature Specification (`features.md`)

**What EventHub does, expressed as traceable, testable behavior.**
...
- epics and features;
- personas served, dependencies, and delivery phase;
- user-visible behavior and acceptance criteria;
- the current delivery-status snapshot.

Product intent and scope belong to [`product.md`](product.md). Architecture, domain rules, concurrency, persistence, and other implementation mechanisms belong to [`technical.md`](technical.md).

### 0.2 Feature record format

Every feature uses a stable `F-<epic>.<n>` identifier and contains:

1. a title and concise capability statement;
2. phase, delivery status, served personas, and dependencies;
3. optional user story or explanatory note;
4. acceptance criteria stated as observable outcomes.

Acceptance criteria **MUST** remain implementation-independent. Each implemented feature **MUST** be backed by automated verification at the lowest appropriate level defined in `technical.md`.

### 0.3 Phase and delivery status

- **MVP** — required for the smallest complete create → sell → deliver → check-in product.
- **Next** — valuable expansion after the critical path works.
- **Later** — deferred enhancement that must not complicate the core prematurely.

Delivery status is separate from phase:

- **implemented** — explicitly marked implemented in the supplied roadmap as of 2026-07-11; this does not by itself mean fully verified.
- **not-confirmed** — a specification exists, but the supplied material did not explicitly confirm implementation.
- **planned** — specified for future delivery and not represented as implemented.

### 0.4 Dependency rule

A feature **MUST NOT** be treated as ready for implementation until all listed dependencies are implemented or an explicit temporary substitute is documented in `technical.md`. Changes to a dependency **MUST** trigger review of every downstream feature.

---

## 1. Build spine

> **EP-1 Accounts → EP-2 Events → EP-3 Ticketing → EP-4 Discovery → EP-5 Purchase → EP-6 Payment → EP-7 Delivery → EP-8 Check-in**, then **EP-9 Audience & Results**, **EP-10 Transfer & Returns**, and **EP-11 Realtime Monitoring** build on the data and tickets produced by the core path.
...
- _Note:_ avatar is an image asset (stored as described in `technical.md` → “Persistence and data ownership” / “Runtime components and ports”).
...
- _Note:_ the image is stored as an object (see `technical.md` → “Persistence and data ownership” / “Runtime components and ports”); the event keeps a reference, not the bytes.
...
- **Acceptance criteria**
  ...
  - A user without the Owner role is refused with an "insufficient permissions" message.
- _Note:_ optional; kept out of the core to protect simplicity (`product.md` QG-1).
...
- **Acceptance criteria**
  ...
  - A free event is free to create and to attend (`product.md` DEC-1).
...
- **Acceptance criteria**
  ...
  - No platform fee is added at any step (`product.md` DEC-1).
...
- _Note:_ the consistency mechanism is in `technical.md` → “Consistency, transactions, and messaging”.
...
- **Acceptance criteria**
  - On a typical phone screen, the page is readable and the buy flow is usable without horizontal scrolling or zooming (`product.md` QG-4).
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-4.1, F-3.4`
From the event page, an attendee picks ticket types and quantities.

- _Story:_ As an attendee, I want to choose my tickets so that I can buy them.
- **Acceptance criteria**
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-5.1`
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-5.1, F-3.4`
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-5.3, F-3.3`
...
`MVP · delivery implemented · serves PER-A1 · depends on F-5.3`
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-5.3`
...
- _Story:_ As an attendee, I want to pay securely so that I receive my tickets.
- **Acceptance criteria**
  - Given a Pending order with a non-zero total, when I choose to pay, then I am taken to the provider to complete payment.
  - EventHub never collects or stores card details (`product.md` QG-6).

#### F-6.2 — Free-order auto-confirm

`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-3.2, F-5.3`
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-6.1`
...
`MVP · delivery implemented · serves PER-A1 · depends on F-6.1, F-5.5`
...
`MVP · delivery implemented · serves PER-O1, PER-O2 · depends on F-6.1`
EventHub takes no cut and holds no money.

- **Acceptance criteria**
  - The amount charged equals the displayed total with no EventHub fee added; funds settle to the organizer through the provider, not through EventHub (`product.md` DEC-1).
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-6.3, F-6.2`
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-7.1`
...
- **Acceptance criteria**
  ...
  - Delivery happens reliably even if it is processed shortly after confirmation (handled asynchronously — see `technical.md` → “Persistence and data ownership” / “Runtime components and ports”).
...
`MVP · delivery implemented · serves PER-A1, PER-A2 · depends on F-7.3`
...
`Next · delivery implemented · serves PER-A1, PER-A2 · depends on F-7.2`
...
`Later · delivery implemented · serves PER-A1, PER-A2 · depends on F-1.4, F-7.1`
...
`MVP · delivery implemented · serves PER-O1, PER-O2 · depends on F-7.1, F-2.4`
Staff scan a QR to admit an attendee.

- _Story:_ As an organizer at the door, I want to scan tickets so that only valid holders enter.
- **Acceptance criteria**
...
`MVP · delivery implemented · serves PER-O1, PER-O2 · depends on F-8.1`
...
- _Note:_ optional; included only if door conditions demand it (kept out of the core for simplicity).
...
`Next · delivery implemented · serves PER-O1, PER-O2 · depends on F-5.2, F-7.1, F-8.1`
The organizer sees everyone who bought or attended.

- _Story:_ As an organizer, I want my attendee list so that I own my audience relationship (`product.md` G-5).
- **Acceptance criteria**
...
`Next · delivery implemented · serves PER-O1, PER-O2 · depends on F-9.1`
...
`Next · delivery implemented · serves PER-O1, PER-O2 · depends on F-3.1, F-6.3, F-8.1`
A simple results view per event.

- **Acceptance criteria**
  - Given I hold the Owner or Staff role with Reporting permission for the event (F-1.5), I can see tickets sold by type, total revenue (which equals gross, since there is no platform fee), the check-in rate, and the number of no-shows — the figures behind the North Star (`product.md` → “North Star”).
  - A user without Reporting permission is refused with an "insufficient permissions" message.

#### F-9.4 — Organizer events overview

`Next · delivery implemented · serves PER-O1, PER-O2 · depends on F-2.1, F-9.3`
...
`Later · delivery implemented · serves PER-O1, PER-O2 · depends on F-9.1, F-7.2`
Email an event's attendees.

- **Acceptance criteria**
  - Given I hold the Owner role for the event (F-1.5), when I send a message to an event's attendees (for example a reminder, update, or cancellation notice), then it is delivered to their emails (processed asynchronously — see `technical.md` → “Persistence and data ownership” / “Runtime components and ports”).
  - A user without the Owner role (including Staff) is refused with an "insufficient permissions" message.

#### F-9.6 — Automatic event reminder

`Later · delivery implemented · serves PER-O1, PER-A1 · depends on F-9.5`
...
- _Story:_ As an attendee who can no longer attend, I want to pass my ticket to a friend so that it is not wasted.
- **Acceptance criteria**
...
- **Acceptance criteria**
  - Given I hold the Owner or Staff role with Reporting permission for the event (F-1.5), when tickets are bought, then my open event view updates the sold count and remaining availability without a manual refresh (delivered in real time — see `technical.md` → “Persistence and data ownership” / “Runtime components and ports”).
...
This is one of exactly three authoritative EventHub specifications. Separate feature roadmaps, epic MOCs, and per-feature documents **MUST NOT** restate or override acceptance criteria. Temporary implementation plans may reference `F-*` identifiers, but durable behavior changes belong here.
