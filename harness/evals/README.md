...
```
harness/evals/
  Invoke-HarnessEvals.ps1 # entry (pwsh or Windows PowerShell)
...
```json
{
  ...
  "run": {
    "type": "hook | command",
    "script": ".codex/hooks/Invoke-PreToolGuard.ps1",
...
.\harness\evals\Invoke-HarnessEvals.ps1 -CaseId graph-affected-domain-users
.\harness\evals\Invoke-HarnessEvals.ps1 -Json
```
...
| Change | Run |
|--------|-----|
| `.codex/hooks/**`, `hooks.json` | `.\harness\evals\Invoke-HarnessEvals.ps1 -Layer harness` |
| `harness/orchestrator/**`, `harness/policies/**`, `harness/telemetry/**`, `harness/tools/**` | `.\harness\evals\Invoke-HarnessEvals.ps1 -Layer harness` |
| `harness/manifest.json`, runtime contract artifacts | `.\harness\evals\Invoke-HarnessEvals.ps1 -CaseId harness-runtime-status` |
| `harness/graph/index.json`, `Get-AffectedTests.ps1` | `.\harness\evals\Invoke-HarnessEvals.ps1 -Layer graph` |
...
3. Run `.\harness\evals\Invoke-HarnessEvals.ps1 -CaseId <new-id>` until green.
...
| **graph** | `Get-AffectedTests.ps1` + `harness/graph/index.json` mapping |
