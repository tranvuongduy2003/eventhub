...
# Observational only; cannot block compaction. See context-memory.mdc and .cursor/notes/
...
$projectRoot = if ($env:CURSOR_PROJECT_DIR) { $env:CURSOR_PROJECT_DIR } else { (Get-Location).Path }
$notesDir = Join-Path $projectRoot '.cursor\notes'
...
$memoryDir = Join-Path $projectRoot '.cursor\agent-memory'
...
$relDir = ".cursor/notes/backups/$stamp-$convShort"
